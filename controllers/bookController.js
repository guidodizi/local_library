...
exports.book_update_get = function(req, res, next) {
    async.parallel({
        ...
        for (var iter_genres = 0; iter_genres < results.genres.length; iter_genres++){
            for (var iter = 0; iter < results.book.genre.length ; iter++){
                if (results.genres[iter_genres]._id.toString() == results.book.genre[iter]._id.toString()){
                    results.genres[iter_genres].checked = 'true';
                }
            }
        }
...
exports.book_update_post = [
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()){
            ...
            //Data from book is valid
            Book.findByIdAndUpdate(req.params.id, book, function(err, new_book){
                if (err) {return next(err);}
                //success
